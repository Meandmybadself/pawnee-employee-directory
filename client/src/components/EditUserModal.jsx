...
class EditUserModal extends React.Component {
  ...
  render() {
    ...
    return (
      ...
      >
        <div className="md:w-6/12 bg-white flex flex-col p-6 md:p-10 rounded-lg relative" style={{ height: '75vh' }}>
          <div
            className="rounded-full bg-orange-600 text-white font-black absolute top-0 right-0 w-6 h-6 md:w-12 md:h-12 flex justify-center items-center m-5 cursor-pointer"
            onClick={() => closeModal()}
          >
            x
          </div>
          <h1 className="font-bold md:text-5xl">Edit User</h1>
          {textFields.map(key => (
            <input
              key={key}
              type="text"
